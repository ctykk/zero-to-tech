...
export default function HomePage() {
  ...
  return (
    <AnimatedCardGrid className="grid grid-cols-12 gap-[18px]">
      ...
      <section
        className="animate-card-enter rounded-panel bg-surface shadow-card col-span-12 grid min-h-[220px] content-center gap-2 p-6 py-7 pr-7"
        data-card=""
      >
        <p className="text-textMuted mb-3 text-xs leading-[1.33] font-semibold tracking-[-0.12px]">
          作品
        </p>
        <p className="max-w-[720px] text-[clamp(34px,4.2vw,52px)] leading-[1.04] font-semibold tracking-[-0.05em] sm:text-[24px]">
          文字实验室
        </p>
        <p className="text-textSoft max-w-[520px] text-[clamp(17px,2vw,21px)] leading-[1.28] tracking-[-0.02em]">
          拼音和情绪，挖掘中文里的细节
        </p>
        <a
      ...
      >
        <div className="pt-1">
          ...
          {identityError ? (
            ...
            <p className="max-w-[520px] text-[clamp(20px,2.2vw,28px)] leading-[1.4] font-medium tracking-[-0.02em]">
              {motto}
            </p>
          )}
        </div>
